...
For example looking at label 19 and 21 (extracted by a simple threshold on the cell scalars generated by \code{vtkDiscreteMarchingCubes}) which are adjacent to each other but mostly separated by the background label, see Fig.~\ref{fig:dmcCt21t19}.
...
\\
\footnotesize
Meshes generated by \code{vtkDiscreteMarchingCubes} for the label image shown above, coloured differently and smoothed with \code{vtkWindowedSincPolyDataFilter}. Note the capping at constrictions.
}
\label{fig:dmcC}
\end{figure}
...
\\
\footnotesize
Result only displaying those regions of the meshes that were adjacent to the background label. Note that the capping is removed compared to Fig. 2
}
\label{fig:dmcO}
\end{figure}
...
\begin{figure}
\center
\includegraphics[width=\textwidth]{images/vtkDiscreteMarchingCubes_extension_03}
\itkcaption[]{
\\
\footnotesize
Showing only the meshes of label 21 (light grey) and 19 (dark grey). Regions of the mesh of label 21 are coloured if adjacent to other labels, e.g. red where 19 and 21 touch, i.e. where the separation took place.
}
\label{fig:dmcCt21t19}
\end{figure}
...
\\
\footnotesize
Views of label 21 only, coloured as in Fig. 4
}
\label{fig:dmcCt21}
\end{figure}
